...
const AssignUserToEmployee = () => {
  ...
  const assignTasksUsingAI = async () => {
    if (!selectedTaskForAI) {
      handleSnackbarOpen("Please select a task to assign using AI");
      return;
    }
  
    setAiLoading(true);
    try {
      // Prepare user data with current tasks
      const usersWithTasks = users
        .filter(user => user.role === "employee")
        .map(user => ({
          name: user.name,
          currentTasks: tasks
            .filter(task => task.employee?._id === user._id)
            .map(task => ({
              title: task.title,
      ...
      // Prepare the prompt
      const promptData = {
        prompt: `You are an intelligent assistant helping assign tasks to users. Here is the data:\n\n1. Unassigned Task:\n- Title: ${selectedTaskForAI.title}\n- Description: ${selectedTaskForAI.description}\n\n2. Users and their current workload:\n${JSON.stringify(usersWithTasks, null, 2)}\n\nBased on the workloads and task relevance, recommend which user should be assigned the unassigned task. Explain your reasoning briefly.`
      };
      ...
      const response = await fetch("https://api.groq.com/v1/chat/completions", {
        method: "POST",
        headers: {
          "Authorization": "Bearer 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",
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          model: "diepngroqchattask",
      ...
      handleSnackbarOpen("Failed to get AI assignment suggestion");
    } finally {
      setAiLoading(false);
    }
  };
